...
from water_forecasting.decisions import should_promote
...
def load_training_frame(spark, cfg: ProjectConfig) -> pd.DataFrame:
    ...
    labels = spark.table(cfg.table(Tables.SILVER_CONSUMPTION)).selectExpr(
        "zone_id", "reading_ts AS target_ts", "consumption_m3"
    )
    ...
    with mlflow.start_run(run_name=f"{cfg.env}-{trigger}-{pd.Timestamp.now():%Y%m%d-%H%M}") as parent:
        ...
        for family in cfg.model.candidates:
            with mlflow.start_run(run_name=family, nested=True) as child:
                est = make_model(family, cfg.model.ridge_alpha, cfg.model.lgbm_params).fit(tr[cols], tr.consumption_m3)
                pred = est.predict(ho[cols])
                metrics = evaluate(ho.consumption_m3, pred)
                for z in cfg.zones:
                    m = ho.zone_id == z
                    metrics[f"mape_{z}"] = evaluate(ho.consumption_m3[m], pred[m.to_numpy()])["mape"]
                mlflow.log_params({"family": family, **{f"p_{k}": v for k, v in est.get_params().items()}})
                mlflow.log_metrics(metrics)
                if family == "lightgbm":
                    imp = est.feature_importance(cols).head(20)
                    mlflow.log_dict(imp.round(1).to_dict(), "feature_importance_top20.json")
        ...
        # Champion vs challenger on the same, most recent holdout.
        champ_model, champ_mv = load_alias(cfg, "champion")
        champ_mape = None
        if champ_model is not None:
            champ_mape = evaluate(ho.consumption_m3, champ_model.predict(ho[cols]))["mape"]
        promote, reason = should_promote(
            float(best.mape), champ_mape, cfg.decisions.min_improvement_pct, cfg.decisions.max_acceptable_mape
        )
        if promote:
            set_alias(cfg, "champion", mv.version)
        mlflow.set_tags({"selected_family": best.family, "promoted": str(promote), "decision": reason})
        mlflow.log_metrics(
            {"challenger_mape": float(best.mape), **({"champion_mape": champ_mape} if champ_mape is not None else {})}
        )
